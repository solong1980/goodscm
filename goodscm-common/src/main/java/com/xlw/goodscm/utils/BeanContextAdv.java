...
public class BeanContextAdv {
	...
	public <T> List<T> scan(String pkg) throws IOException {
		String path = pkg.replace(".", File.separator);
		// URL resource = BeanContextAdv.class.getClassLoader().getResource(".");
		Enumeration<URL> resources = BeanContextAdv.class.getClassLoader().getResources(".");
		...
		while (resources.hasMoreElements()) {
			...
			if (listFiles == null) {
				try {
					Enumeration<URL> urlEnumeration = Thread.currentThread().getContextClassLoader().getResources(pkg.replace(".", "/"));
